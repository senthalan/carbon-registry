...
public class RegistryMgtUISearchServiceComponent {
    ...
    protected void activate(ComponentContext context) {
        ...
        serviceRegistration = context.getBundleContext().registerService(MetadataSearchService.class.getName(),
                metadataSearchService, null);
        try {
            QueryProcessorManager queryProcessorManager = dataHolder.getRegistryService().getRegistry(CarbonConstants
                    .REGISTRY_SYSTEM_USERNAME).getRegistryContext().getQueryProcessorManager();
            if (queryProcessorManager.getQueryProcessor(XPathQueryProcessor.XPATH_QUERY_MEDIA_TYPE) == null) {
                ...
                queryProcessorManager.setQueryProcessor(XPathQueryProcessor.XPATH_QUERY_MEDIA_TYPE, new
                        XPathQueryProcessor(metadataSearchService));
            }
    ...
    private static class MetadataSearchServiceImpl implements MetadataSearchService {
        ...
        public ResourceData[] search(UserRegistry registry, Map<String, String> parameters) throws RegistryException {
            ...
            AdvancedSearchResultsBean resultsBean = AdvancedSearchResultsBeanPopulator.populate(null, registry,
                    parameterBean);
        ...
        public ResourceData[] search(int tenantId, Map<String, String> parameters) throws RegistryException {
            ...
            return search(SearchDataHolder.getInstance().getRegistryService().getRegistry(CarbonConstants
                    .REGISTRY_SYSTEM_USERNAME, tenantId), parameters);
        }
